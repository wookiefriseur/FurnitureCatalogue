...
local function addItemLink(data)
  ...
  if data.itemType == ITEMTYPE_FURNISHING then
    ...
    if data.specializedType == SPECIALIZED_ITEMTYPE_FURNISHING_CRAFTING_STATION then
      return
    end
    ...
    if recipeKey then
      ...
      sidTools_SaveData.furniture[recipeKey] =
        string.format("rumourSource, -- %s", GetItemLinkName(recipeResultItemLink))
